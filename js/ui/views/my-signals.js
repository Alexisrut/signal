 ...
 * Изолированная среда: список строится только по authorId текущего пользователя.
 */

import { html, formatDateTime, truncate } from '../../core/utils.js';
import { STATUS, STATUS_META, STATUS_ORDER, lineLabel } from '../../core/constants.js';
import { currentActor } from '../../domain/auth.js';
import { listByAuthor, getForActor, changeStatus, countByStatus } from '../../domain/signals.js';
...
function resolveSignal(signalId, actor) {
  const signal = getForActor(signalId, actor);
  return signal;
}

function bindResolveButtons(root, actor, onDone) {
  root.querySelectorAll('[data-resolve]').forEach((button) => {
    button.addEventListener('click', () => {
      const result = changeStatus(button.dataset.resolve, STATUS.GREEN, actor);
      if (!result.ok) {
...
export const mySignalsView = {
  ...
  render() {
    ...
    const signals = listByAuthor(actor.id);
    ...
    const chips = STATUS_ORDER.filter((status) => counters[status] > 0).map(
      (status) => html`<span class="chip chip--static chip--${status}">${STATUS_META[status].label}: ${counters[status]}</span>`,
    );
  ...
  mount(root, ctx) {
    bindResolveButtons(root, currentActor(), ctx.refresh);
  },
};
...
export const mySignalView = {
  ...
  render(ctx) {
    const actor = currentActor();
    const signal = resolveSignal(ctx.params.id, actor);
          ...
          <div class="detail__actions">
            ${[
              canResolve
                ...
                : html`<span class="detail__note">${STATUS_META[signal.status].terminal
