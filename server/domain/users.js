...
/**
 * Кандидаты в ответственные за сигнал — весь штат, кроме подрядчиков.
 *
 * Раньше список ограничивался руководителями, из-за чего администратора нельзя
 * было назначить исполнителем даже на собственную задачу. Теперь в выборе есть
 * и администраторы, и главные администраторы.
 *
 * Отбор по категории остается, но применяется не ко всем: окно распределения
 * прогоняет список через canCurate, и там руководитель отсеивается, если за ним
 * не закреплена категория сигнала, а администратор проходит всегда.
 *
 * Порядок — по рангу роли, внутри роли по алфавиту: в окне выбора сначала идут
 * администраторы, затем руководители.
 */
export function listAssignables() {
  return sql
    .all(`SELECT * FROM users WHERE role IN (?, ?, ?)`, [ROLE.SUPERADMIN, ROLE.ADMIN, ROLE.MANAGER])
    .map(toUser)
    .map((user) => ({
      id: user.id,
      role: user.role,
    ...
    .sort(
      (a, b) =>
        roleRank(a.role) - roleRank(b.role) || a.displayName.localeCompare(b.displayName, 'ru'),
    );
}
